...
const Navbar = () => {
  ...
  return (
    <>
    <div className='relative bg-[#E9FCE9] z-50'>
      <nav className='container mx-auto flex items-center justify-between p-5'>
        <Link to='/' className='text-green-600 font-bold text-xl'>Tern
      ...
      <ul className='hidden lg:flex gap-10 text-base'>
        ...
        <li>
          <Link to="/">Top Jobs</Link>
        </li>
        <li>
          <Link to="/upload-job">Create Job</Link>
        </li>
        <li>
          <Link to="/about-us">About Tern</Link>
        </li>
      </ul>
      <div className='hidden lg:block'>
        {!user?.token ? (
            <Link to='/user-auth'>
             <CustomButton title="Sign-In" containerStyles='text-green-600 py-1.5 px-5 focus:outline-none hover:bg-green-700 hover:text-white rounded-full text-base border border-green-600'/> 
            </Link>
          ):(
            <div>
              <MenuList user={user}/>
